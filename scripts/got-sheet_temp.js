...
class GOTActorSheet extends ActorSheet {
    ...
    _prepareUnitData(context) {
        ...
        // Initialize Moral if missing
        if (m.moral === undefined) m.moral = m.vigor || 2;
...
class GOTBattleHUD extends Application {
    ...
    getData() {
        ...
        const dynamicManeuvers = (maneuversMap[m.tipo] || maneuversMap["Infantaria"]).map(man => {
            ...
            return { ...man, met: combined >= man.req };
        });
        ...
        const disciplineMax = Number(m.vigor || 2);
    ...
    activateListeners(html) {
        ...
        html.find('.recovery-btn').click(async ev => {
            ...
            await roll.toMessage({ speaker: ChatMessage.getSpeaker({ actor }), flavor: "Tentativa de Reagrupar Unidade" });
